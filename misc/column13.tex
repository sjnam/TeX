...
you can make one using a function {\it rand\/} that returns a random real
distributed uniformly in $[\,0,1)$ by the expression $l+{\bf int}(rand\times
(u+1-l)).$ In the unlikely event that your system doesn't even have that
routine, consult Knuth's {\sl Seminumerical Algorithms.\/} But whether you use
a system routine or make your own, be careful that {\it rand\_int\/} returns
a value in the range $l\ltdot u$ --- a value out of range is nasty bug.}
