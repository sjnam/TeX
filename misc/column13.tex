...
\font\clfont=cminch at 16pt
\letterspacefont\columnfont=\clfont -50
...
\def\lheader{\sc{\ninerm\the\pageno}\quad more programming pearls\hfill\colnum}
\def\rheader{\sc\colnum\hfill\colname\quad{\ninerm\the\pageno}}
...
u.$\footnote\dag{\eightpoint If you don't have a {\it rand\_int\/} function,
you can make one using a function $Rand$ that returns a random real distributed
uniformly in $[\,0,1)$ by the expression $l+{\bf int}(rand\times(u+1-l)).$ In
the unlikely event that your system doesn't even have that routine, consult
Knuth's {\sl Seminumerical Algorithms.\/} But whether you use a system routine
or make your own, be careful that {\it rand\_int\/} returns a value in the
range $l\ltdot u$ --- a value out of range is nasty bug.}
...
\+\qquad {\bf return} the empty set\cr
\+\quad\bf else\cr
