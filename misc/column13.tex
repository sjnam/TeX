...
\begindisplay
\vbox{
\+\bf BEGIN $\{$ \cleartabs&$m={\tt ARGV}[1]$; $n={\tt ARGV}[2]$\cr
\+&\bf for \cleartabs&$(\,j=n-m+1$; $j\le n$; $j\PP)\ \{$\cr
\+&& $t=1+{\bf int}(j*{\it rand}())$\cr
\+&&\bf if $(t\ {\bf in}\ s)\ s[j] = 1$\cr
\+&&\bf else $s[t] = 1$\cr
\+&$\}$\cr
\+&\bf for $(i\ {\bf in}\ s)$ {\it print} $i$\cr
\+$\}$\cr}
\enddisplay
