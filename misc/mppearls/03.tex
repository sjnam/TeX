...
of $x$ so that $x[1\ltdot k-1]\le x[k]\le x[k+1\le n].$ We will study this
...
That program uses ``tail recursion'': the recursive call is last statement in
the procedure. Tail recursion can always be transformed to iteration by
...
\begindisplay
\vbox{
\+\bf function ${\it swap}(i,j,t)\ \{\ t\leftarrow x[i];\ x[i]\leftarrow x[i];\ x[j]\leftarrow t\ \}$\cr
...
\endgroup}
\enddisplay
Although the code produces the correct answer, the trace is supicious. Can you
...
Apart from its correctness problem, the origianl code ahs a ``performance bug'':
...
Before this column was originally published in {\it Communications of the
ACM\/}, many programmers had mentioned that they used pseudocode published in
previous columns as a basic for implementing an algorithm i their favorite
...
interfaces; all routines operate on the array $x[1\ltdot n].$ There shot, clean,
correct routines provide a useful starting point for programmers without a
better library.
...
{\it The Mythical Man Month}, Fred Brooks states that there might be ``half as
much code in scaffolding as there is in product''; in Section 1.4.1 of {\it
...
arguemtns using techniques of program verification. Ihad tested all the routines
before publication, using a combination of watching, measuring and black-box
testing; some columns report bugs I found during that process. I therefore
wasn't suprised when testing uncovered no logical errors in the routines; I
fixed a few syntax errors in less than a minute each.
...
3.1, the infinite loop was obvious. I presented the resulting fifteen linee to
...
numbers, otherwise they are compared as strings. Becasuse of unusaly
...
\prob
Write scaffolding that allows you to tiem the performance of various algorithms
...
\prob
By teh literal specifications in Appendix 2, thsi is a correct sorting
algorithm:
\begindisplay
\vbox{
