...
이 문서는 실비오 레비가 \CEE/에 적용한 돈 크누스의 \.{WEB} 시스템의 한 버전을 설명한다.
...
크누스의 메모 ``구조적 문서화의 \.{WEB} 시스템''이란 문서에 익숙한 독자는 이 문서를
...
이름({\sl name\/})으로 인식된다. 섹션 이름은 `\.{@<}', \TEX/ 텍스트, `\.{@>}' 순으로
...
\def\headlinefont{\rm\hfont{outgrm}{at 9pt}}
...
파일 \.{cwebmac}에 정의되어 있는 매크로들 이용하면 \.{CWEAVE}의 결과물인 \TEX/ 코드들을
...
\point 4. 출력 페이지의 크기를 변경하고자 한다면, 작성하는 \.{CWEB} 파일의 림보에 다음과
...
\point 10. \.{CWEAVE}의 결과물을 개별적으로 처리 가능한 여러 개의 서브
...
섹션 이름을 볼 수 있다. 따라서 Acrobat 사용자는 이 기능을 통해서
