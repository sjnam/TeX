...
\smallskip\noindent
전자 형식의 이 문서의 저작권은 실비오 레비와 도널드 크누스에게 있다.
...
책인 {\sl 문학적 프로그래밍\/} (1992)은 문학적 프로그래밍과 관련된 많은 참고 문헌들과
초창기 작업의 풍부하고 흥미로운 내용을 담고 있다. 보데, 슈니터, 마하파트라는 \.{CWEB}에서
\CPLUSPLUS/도 사용할 수 있도록 하여서, 독자들은 원하기만 하면, 이 글에서 \CEE/를
\CPLUSPLUS/로 바꾸어 생각해도 된다.
...
\.{CWEB} 파일은 마디({\sl section\/})라고 하는 하나의 온전한 의미가 있는 단위들로 구성된다.
각 마디는 다음과 같은 세 개의 부분을 가지고 있다.
%부(部, part)를 가지고 있다.
...
\section 제어 부호.
\noindent
\.{CWEB}의 제어 부호({\sl control code\/})는 첫 번째 문자가 `\.@'인 두 글자의
...
%restricted.
대괄호 안에는 위에서 설명한 것들 외에 두 가지 다른 문자가 나올 수 있는데, 글자 $r$은 제한된
환경({\it restricted context\/})을 나타내는데, \CEE/ 주석문 안의 환경, 마디 이름,
...
짝을 이루고 그러한 명령어들로 이루어지는 환경이 제한된(restricted) 환경이라는 뜻이다.
