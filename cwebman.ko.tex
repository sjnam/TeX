...
\sethangulfont\authorhangul="Noto Serif CJK KR:%
  language=KOR;script=hang;interhangul=-0.04em;interlatincjk=.125em;%
  charraise=-0.06em;+compresspunctuations" at 11pt
...
% The original squeezes .5pt out of \baselineskip to save pages; Korean needs
% the leading more than the manual needs the space, so we keep the full 14pt.
...
\def\lheader{\mainfont\the\pageno\hfill\sc\ninegt\runninghead\hfill}
\def\rheader{\hfill\sc\ninegt\runninghead\hfill\mainfont\the\pageno}
...
\printmacs
\endgroup
% \titletrue suppresses the running head on the page that follows, the way the
% openings of appendices A and B do it.  The original leaves it out here, so
% Appendix C alone starts with a head on its first page; this is that slip
% mended, and the one place the translation departs from cwebman.tex.
...
\point 8. 매크로 \.{\\iftitle}은 `\.{\\titletrue}'로 정의되면 머리글 줄을
...
번역본이 쓰고 있는 {\tt kotexcweb.tex}에 들어 있다. 그 파일은 위의 매크로들과
함께 한글 글꼴과 러닝 헤드, 목차와 절 이름 목록의 제목까지 손보며, luatex으로
조판한다. {\tt CWEB} 파일의 limbo 첫 줄에 {\tt \string\input} {\tt kotexcweb}이라고 적으면
된다.}
