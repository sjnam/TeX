...
\section 제어 부호.
\noindent
\.{CWEB}의 제어 부호({\sl control code\/})는 첫 번째 문자가 `\.@'인 두글자의 결합이다.
이미 앞에서 여러 제어 부호를 살펴봤는데, 이제 \.{CWEB}의
모든 제어 부호를 좀 더 순차적으로 나열해 볼 시간이 됐다.
...
앞으로 나열하는 리스트에서 제어 부호 다음에 나오는 대괄호 안의
글자는 그 제어 부호가 어떤 환경에서 사용될 수 있는가를 설명한다.
$L$은 명령어가 in limbo에서 사용될 수 있다는 것을 뜻하고,
$T$는 \TEX/부에서, $M$는 중간부에서, $C$는 \CEE/언어부와 같은 최상위
수준에서 사용될 수 있다는 것을 나타낸다. 최상위 수준이라 함은, `\pb'나
마디 이름과 같은 환경 이외의 것을 말한다. 화살표 $\to$는 제어 부호를 사용함으로
해서 현재 환경을 마감하고 화살표 다음에 나오는 글자가 뜻하는 새로운
환경이 시작된다는 것을 의미한다. 따라서 제어 부호 \.{@\ } 다음에 나오는
$[LTMC\to T]$가 뜻하는 바는 이 제어 부호는 in limbo와 마디의 \TEX/부,
중간부, \CEE/언어부에서 사용할 수 있고, 이 제어 부호를 사용하면,
...
설명하는 제어 부호는 \.{@>}와 짝을 이루고 그러한 명령어들로 이루어지는
...
%specifically below.
\.{CWEB} 제어 부호를 구성하는 두 문자중 `\.{@}'를 제외한 나머지 한 문자가
...
나타난다. 이 제어 부호는 모든 환경에서 사용할 수 있는 유일한 것이며, 
...
\subsec
다음 제어 부호를 사용하면 마디의 \TEX/부가 시작된다.
...
\@* [LTMC\to T] 이 제어 부호를 사용하면 새로운 별표 마디가 시작된다. 별표
