\directlua{%
  local fadelines = function (head)
     local colorize = node.new("whatsit", "pdf_colorstack")
     local cvalue = 0
     %for line in node.traverse_id(node.id("glyph"), head) do     
     for line in node.traverse_glyph(head) do
  ...
  callback.register("pre_linebreak_filter", fadelines)
}
